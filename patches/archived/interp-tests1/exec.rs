    ...
    async fn await_with_timeout(
        ...
        match tokio::time::timeout(timeout, &mut *handle).await {
            ...
            Err(_elapsed) => {
                ...
                // NOTE(interp): This silently discards ALL buffered output.
                // The model receives empty text with no truncation signal.
                tracing::warn!(
                    timeout_ms = IO_DRAIN_TIMEOUT_MS,
                    "IO_DRAIN_TIMEOUT: pipe still open after {}ms, \
                     aborting read task — all buffered output DISCARDED silently",
                    IO_DRAIN_TIMEOUT_MS,
                );
                handle.abort();
                Ok(StreamOutput {
...
mod tests {
    ...
    /// Exercises a tool (frep) that spawns a background index process after
    /// returning search results. The background process holds stdout open,
    /// which can trigger the IO_DRAIN_TIMEOUT. This test logs what the exec
    /// layer actually returns so the behavior is observable.
    #[cfg(unix)]
    #[tokio::test]
    async fn frep_with_background_indexer() -> Result<()> {
        // Create a small file to search.
        let tmp = std::env::temp_dir().join("frep_test_input.txt");
        std::fs::write(
            &tmp,
            "line1: hello world\nline2: foo bar\nline3: hello again\n",
        )?;

        // Locate frep relative to the workspace root.
        let workspace = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            ...
            .unwrap()
            .to_path_buf();
        let frep = workspace.join("scripts/frep");

        let command = vec![
            frep.to_string_lossy().to_string(),
            "-n".to_string(),
            "hello".to_string(),
        ...
        let stdout_text = String::from_utf8_lossy(&output.stdout.text);

        // Log exactly what the model would receive.
        eprintln!(
            "frep_with_background_indexer:\n  \
             timed_out={}\n  \
             stdout_bytes={}\n  \
             stdout_text={:?}\n  \
             truncated={:?}",
            ...
            stdout_text,
            output.stdout.truncated_after_lines,
        );

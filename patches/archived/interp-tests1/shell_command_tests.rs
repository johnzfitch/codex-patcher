...
/// Runs `frep` (scripts/frep) through the full shell_command pipeline.
/// frep is a grep-like tool that spawns a background index refresh process.
/// This test logs what the model actually receives as output.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn frep_search_through_shell_command() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    skip_if_windows!(Ok(()));
    ...
    let tmp = std::env::temp_dir().join("frep_shell_test.txt");
    std::fs::write(
        &tmp,
        "fn main() {\n    println!(\"hello\");\n    // TODO: fix this\n}\n",
    )?;
    ...
    .await;
    harness.submit("search for TODO comments").await?;
    ...
    eprintln!(
        "frep_search_through_shell_command:\n  output={normalized:?}"
    );
